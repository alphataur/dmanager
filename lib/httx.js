...
const { truncateParts } = require("../utils")
...
class HTTX extends events{
  ...
  initPipeLines(){
    for(let i = 0; i < this.nParts; i++){
      this.partitions[i].data.on("data", (chunk) => {
        ...
        this.emit("progress", this.makeProgess())
      })
  ...
  async start(){
    ...
    let sources = []
    for(let i = 0; i < this.isCompleted.length; i++){
      sources.push(`${this.dpath}/${this.hash}.part${i}`)
    }
    await truncateParts(sources, this.fpath)
  }
}
